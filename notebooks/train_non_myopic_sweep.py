#!/usr/bin/env python
# ============================================================
# NON-MYOPIC (coupled, full-state) rho-SWEEP — headless training driver
# Trains 2-asset NON-MYOPIC agents (SymmetricActorCriticPolicy, full state) in the
# fully-coupled environment for
#   Trial 8  -> rho = rho_c = 1
#   Trial 9  -> rho = rho_c = 2
#   Trial 10 -> rho = rho_c = 3
#   Trial 11 -> rho = rho_c = 4
#   Trial 12 -> rho = rho_c = 5           (rho = trial_num - 7)
#
# Saved EXACTLY like train_myopic_sweep.py, but with "non_myopic" in the names
# (tag = "2Assets_non_myopic", run_name = "PPO_2Assets_non_myopic_Trial{n}"),
# so it does NOT collide with or delete any existing checkpoints/metadata
# (neither PPO_2Assets_Trial8-12 nor the myopic PPO_2Assets_Myopic_Trial*).
#
# Run in a TERMINAL (AC power, Dropbox paused):
#   conda activate N_mbt_gym_arm
#   caffeinate -i python notebooks/train_non_myopic_sweep.py 2>&1 | tee train_non_myopic_sweep.log
#
# TensorBoard (separate terminal) picks up each run automatically:
#   tensorboard --logdir ./N_tensorboard
#
# Set SWEEP_SMOKE=1 to dry-run (verify rho per trial, save NOTHING).
# ============================================================
...
from mbt_gym.agents.SymmetricPPOPolicy import SymmetricActorCriticPolicy   # full-state (non-myopic)
...
SWEEP = [(8, 1.0), (9, 2.0), (10, 3.0), (11, 4.0), (12, 5.0)]
...
def train_trial(trial_num, rho):
    tag = f"{num_assets}Assets_non_myopic"
    run_name = f"PPO_{num_assets}Assets_non_myopic_Trial{trial_num}"
    cross = [[0.0, rho], [rho, 0.0]]
    c_cross = [[0.0, rho], [rho, 0.0]]

    print("\n" + "=" * 64)
    print(f" TRIAL {trial_num}  |  rho = rho_c = {rho}  |  {run_name}  (NON-MYOPIC / full-state)")
    print("=" * 64)
...
if __name__ == "__main__":
    ...
    print(f"\n===== NON-MYOPIC SWEEP COMPLETE in {(time.time()-t_all)/3600:.2f} h =====")
